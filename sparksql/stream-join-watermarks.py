...
transactions_stream.createOrReplaceTempView("txn")
status_stream.createOrReplaceTempView("status")

joined_stream = spark.sql("""
    SELECT
        t.user_id,
        t.transaction_id,
        s.status,
        t.event_time AS txn_ts,
        s.event_time AS status_update_ts
    FROM txn t
    JOIN status s ON s.transaction_id = t.transaction_id
        AND t.event_time BETWEEN s.event_time - INTERVAL 5 SECONDS AND s.event_time + INTERVAL 5 SECONDS
""")
